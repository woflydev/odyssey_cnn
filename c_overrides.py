...
MIDDLE_ANGLE = 310 # DEFAULT 635 - use the included reference_feeler to determine the middle angle
...
def detect_obstacles(edges, obstacle_frame):
	...
	#print(f"Reference Feeler Direction: {reference_feeler[1]}")

	potential_obstacle = max(c)

	cv2.imwrite("obstacle.test.png", obstacle_frame)

	adjustment = 0
	
	if (potential_obstacle[0]) > MIN_DISTANCE:
		print(f"Obstacle Importance: {potential_obstacle[0]}")
		...
		if potential_obstacle[1] < MIDDLE_ANGLE: # 635 is the middle
			adjustment += 0.6 + potential_obstacle[0] * 0.003
...
def main():
	...
	while True:
		...
		upper_purple = np.array([179, 255, 86])
